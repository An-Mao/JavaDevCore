...
public class _Log implements _LogCore {
	...
	public _Log(Class<?> name) {
		this(name.getName());
	}

	public _Log(String name) {
		this(name, "");
	}

	public _Log(Class<?> name, String logFile) {
		this(name.getName(), logFile);
	}

	public _Log(String name, String logFile) {
		this(name, logFile, isAnsiSupported());
	}

	public _Log(Class<?> name, String logFile, boolean color) {
		this(
				name.getName(),
				logFile,
				color);
	...
	public _Log(String name, String logFile, boolean color) {
		this(
				name,
				logFile,
				color,
	...
	public _Log(
			...
			String errorColor,
			String debugColor) {
		this(name.getName(), logFile, color, infoColor, warnColor, errorColor, debugColor);
	}
		...
		try (
			...
			while (true) {
				...
				if (pendingCount >= FLUSH_BATCH_SIZE
						|| now - lastFlushTime >= TimeUnit.MILLISECONDS.toNanos(
								FLUSH_INTERVAL_MS)) {
