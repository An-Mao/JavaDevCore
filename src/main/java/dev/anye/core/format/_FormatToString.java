...
public class _FormatToString {
	...
	public static String oldFormatValue(Object value, int decimalPlaces) {
		if (value instanceof Number) {
			double doubleValue = ((Number) value).doubleValue();
			return oldFormatDecimal(doubleValue, decimalPlaces);
		} else if (value instanceof String) {
			try {
	...
	private static String oldFormatDecimal(double value, int decimalPlaces) {
	...
	private static String formatDecimal(double value, int decimalPlaces) {
		...
		if (Math.abs(value) < 1) return String.format("%." + decimalPlaces + "g", value);
		String formattedValue = String.format("%." + decimalPlaces + "f", value);
		if (formattedValue.endsWith(".00")) return String.valueOf((long) value);
